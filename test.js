...
cases(
  'async functions',
  opts => {
    let transformed = babel.transform(opts.code, {
      plugins: [[plugin, { conversion: opts.conversion }]],
    });
  ...
  [
    {
      ...
      name: 'Pig Latin',
      code: `
const weAreImportingThis = require('somethingImported').default
const { spreadable } = weAreImportingThis
import whateverThisIs from 'anNPMmodule'
console.log(spreadable)
console.log(whateverThisIs.anObjectProp.anotherObjectProp)
var baseVariable = 'base variable';
const constant = 'constant variable';
let changeable = 'lettable variable';
function something (feasible, locquacious) {
	let someWords = 'something feasible' + feasible
	return someWords
}
changeable = 'we changed it!'
let someArray = [baseVariable, constant, changeable, something]
const anObject = {
	someName: 'a string',
	baseVariable,
}
const { someName } = anObject
module.exports = {
	something,
	changeable,
}
...
Keystone.prototype.console.err = function (type, msg) {
	if (keystone.get('logger')) {
		var dashes = '------------------------------------------------';
		console.log(dashes + 'KeystoneJS: ' + type + msg + dashes);
	}
};
